...
def _tab_analytics(obs: pd.DataFrame):
    ...
    if "sample_date" in obs.columns and "spec_min" in obs.columns:
        ...
        st.markdown("**Impurity levels** &nbsp; "
                    "<span style='opacity:0.6;font-size:0.85rem;'>"
                    "lower is better; the dashed line shows the median spec "
                    "ceiling — anything trending toward it is drifting</span>",
                    unsafe_allow_html=True)
        if not impurity_df.empty:
            ...
            # Add a horizontal "Spec ceiling" reference line at the median
            # spec_max across all impurities. Acts like an SPC upper control
            # limit — a single visual cue showing where the danger zone is.
            spec_ceiling = impurity_df["spec_max"].dropna().median()
            if pd.notna(spec_ceiling) and spec_ceiling > 0:
                pivot["Spec ceiling"] = spec_ceiling
            st.line_chart(pivot, height=300)
        else:
...
def render_dashboard():
    ...
    # Most-corrected columns — shows where the source process is noisiest
    st.markdown("&nbsp;")
    st.markdown("**Columns needing the most cleaning**")
    st.caption(
        "Top 10 input columns by number of corrections. Helpful for "
        "identifying which fields the source process should tighten up."
    )
    try:
        col_break = run_query(f"""
            SELECT column AS column_name, COUNT(*) AS fixes
            FROM {CATALOG}.silver.dq_issues
            ...
            LIMIT 10
        """)
        if not col_break.empty:
            st.bar_chart(
                col_break.set_index("column_name")["fixes"],
                color=PRIMARY, height=320,
                horizontal=True,
            )
    except Exception:
